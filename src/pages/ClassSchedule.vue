<template>
    <div class="wrapper">
        ...
        <v-btn v-on:click="is_edit = true; $store.dispatch('get_can_register_lectures','kk')"> 授業を登録する</v-btn>
        <v-btn v-on:click="is_edit = false"> 登録を終わる</v-btn>

        <v-tabs v-model="tabs">
            <v-tab v-for="timetable in $store.state.timetables" :key="timetable.id" v-on:click="$store.commit('set_looking_timetable', timetable)"><b>{{timetable.grade}}{{timetable.semester}}</b>
            </v-tab>
        </v-tabs>
        <v-tabs-items v-model="tabs">
            <v-tab-item v-for="timetable in timetables" :key="timetable.id">
                <TimeTableShow :is_edit="is_edit"
                               ...
                               :can_register="get_grade_half_lectures($store.state.can_register_lectures, timetable.grade, timetable.semester)"></TimeTableShow>
            </v-tab-item>
        </v-tabs-items>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            set_now(data) {
                this.$set(this.registered_lectures[data['grade']][data['semester']][data['day']], data['time'], data['now']);
                // this.$emit('GET_NOW');
            },
            put_editor() {
                return axios.put(URL_BASE + '/editor', 'registered_lectures')
                    .then(() => {
                        this.is_show = true;
                        console.log('保存しました');
                    })
